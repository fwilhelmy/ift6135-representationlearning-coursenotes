...
\textbf{Reference:} Ioffe and Szegedy (ICML 2015)\\
For a mini-batch of size \(B\) with \(F\) features per channel, we normalize over each channels in all samples
\[
\mu_c = \frac{1}{B \times F} \sum_{b, f} x_{bcf},\quad \sigma^2_c = \frac{1}{B \times F} \sum_{b, f} (x_{bcf} - \mu_c)^2.
\]

\subsubsection{Layer Normalization} \label{sec:layernorm}
\textbf{Reference:} Ba, Kiros, and Hinton\\
Normalizes over all features for each sample
\[
\mu_b = \frac{1}{C \times F} \sum_{c,f} x_{bcf},\quad \sigma^2_b = \frac{1}{C \times F} \sum_{c,f} (x_{bcf} - \mu_b)^2.
\]

\subsubsection{Instance Normalization}\label{sec:instancenorm}
Normalizes each channel per sample
\[
\mu_{bc} = \frac{1}{F} \sum_{f} x_{bcf},\quad \sigma^2_{bc} = \frac{1}{F} \sum_{f} (x_{bcf} - \mu_{bc})^2.
\]

\subsubsection{RMS Normalization}\label{sec:rmsnorm}
Normalizes using the root mean square
\[
\mu_b^2 = \frac{1}{C \times F} \sum_{c,f} x_{bcf}^2,\quad \hat{x}_{bcf} = \frac{x_{bcf}}{\sqrt{\mu_b^2 + \epsilon}}.
\]

\subsubsection{Group Normalization}\label{sec:groupnorm}
Divides channels into \(G\) groups and normalizes each group separately.

\subsubsection{Weight Normalization}\label{sec:weightnorm}
TODO
...
\end{itemize}
% TODO add the image in the slides of the course
